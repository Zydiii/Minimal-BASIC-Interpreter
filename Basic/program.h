 ...
 */
  ...
  void addSourceLine(int lineNumber, std::string line, TokenScanner &TS);
 ...
 */
  ...
  /*
 * Method: getexecuteLineNumber
 * Usage: int line = program.getexecuteLineNumber();
 * ------------------------------------------------------------
 * Returns executeLine that the programs will execute, if it 
 * reaches the end of programs, this method returns -1.
 */

  int getexecuteLineNumber();

  /*
 * Method: setexecuteLine
 * Usage: program.setexecuteLine(line);
 * ------------------------------------------------------------
 * Set line as executeLine, if the line doesn't exist, this
 * method reports an error named SYNTAX ERROR.
 */

  void setexecuteLine(int line);

  /*
 * Method: initexecuteLine
 * Usage: program.initexecuteLine();
 * ------------------------------------------------------------
 * Initiate executeLine to -1;
 */

  void initexecuteLine();

  /*
 * Method: RenewCurrenLine
 * Usage: program.renewexecuteLine();
 * ------------------------------------------------------------
 * Initiate executeLine to the first line of programs;
 */

  void renewexecuteLine();

private:
  int executeLine;
  map<int, string> programs;
  map<int, Statement *> parsedStatements;
};
